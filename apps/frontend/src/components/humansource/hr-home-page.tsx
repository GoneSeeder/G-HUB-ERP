    ...
    <Panel className="p-4">
      ...
      <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
        <div className="flex items-center justify-between text-xs">
          ...
          <span className="font-semibold text-slate-950">4 ชม. 18 นาที</span>
        </div>
      </div>
    </Panel>
  );
...
function TimeRecord({
  ...
  return (
    <div className={`rounded-lg border px-3 py-3 ${active ? 'border-emerald-200 bg-emerald-50' : 'border-slate-200 bg-white'}`}>
      <div className="flex items-center justify-between gap-2">
        <p className={active ? 'text-xs font-medium text-emerald-700' : 'text-xs font-medium text-slate-500'}>{label}</p>
        <span className={`h-2 w-2 rounded-full ${active ? 'bg-emerald-500' : 'bg-slate-300'}`} />
      </div>
      <p className={`mt-2 text-xl font-semibold ${active ? 'text-emerald-900' : 'text-slate-950'}`}>{time}</p>
      <p className={active ? 'mt-1 text-[11px] text-emerald-700' : 'mt-1 text-[11px] text-slate-500'}>{detail}</p>
      <p className="mt-2 text-[10px] text-slate-400">{state}</p>
    </div>
  );
}
  ...
  return (
    <article className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="flex items-center gap-3 px-4 py-3">
        <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full text-xs font-semibold text-white ${toneClass}`}>
          HR
        </div>
        <div className="min-w-0">
        ...
        <span className="ml-auto rounded-full bg-slate-100 px-2.5 py-1 text-[10px] font-medium text-slate-600">{badge}</span>
      </div>
      <div className="border-y border-slate-100 px-4 py-4">
        <h3 className="text-base font-semibold text-slate-950">{title}</h3>
        <p className="mt-2 text-sm font-light leading-6 text-slate-600">{body}</p>
      </div>
    ...
    <Panel className="p-4">
      ...
      <div className="mt-3 space-y-2">
        {employeeRequests.map((item) => (
          <div key={item.title} className="rounded-lg border border-slate-100 px-3 py-2.5">
